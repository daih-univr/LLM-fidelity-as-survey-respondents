...
def load_model():
    ...
    if "HF_TOKEN" in os.environ:
        print("HF_TOKEN environment variable found. Proceeding with download.")
    else:
...
D: Completamente d'accordo
    ...
    enriched = strip_think(output["choices"][0]["text"]).replace("```json", "").replace("```","").strip()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "file_name", 
        nargs="?", 
        default="/mnt/sharedHD/CodeRepo/simulate_erickson/data/machines_qwen/Q2025_6ottobre2025_finalCleaned.xlsx", 
        type=str, 
    ...
    output_file_name = file_name.replace(".xlsx",".qwen_enriched_output.xlsx")
