...
from openai_harmony import load_harmony_encoding, HarmonyEncodingName, Role, HarmonyError
...
def load_model():
    ...
    print("Model loading complete (transformers).")

    return llm, tokenizer

def harmony_content_to_text(content) -> str:
...
def llm_enrich(profilo: Dict[str, str], llm, tokenizer) -> Dict[str, object]:
...
D: Completamente d'accordo
    ...
    try:
        msgs = encoding.parse_messages_from_completion_tokens(
            new_tokens, Role.ASSISTANT, strict=False
        )
        final_msgs = [m for m in msgs if getattr(m, "channel", None) == "final"]
        content = final_msgs[-1].content if final_msgs else ""
        completion = harmony_content_to_text(content)

    except HarmonyError:
        print("Error parsing Harmony content. Falling back to raw text extraction.")
        completion = tokenizer.decode(new_tokens, skip_special_tokens=True)

    risposta = completion.strip()
    ...
    try:
        ...
        enriched_dict = json.loads(risposta)
        if ruolo not in ["Insegnante di sostegno", "Insegnante curricolare"]:   
            enriched_dict["q18"] = "e: vuota"
            enriched_dict["q19"] = "e: vuota"
        print(f"Output: '{enriched_dict}'")
        print("#"*20)
        ...
        print(f"Raw output causing error: {risposta}")
        return {}
...
if __name__ == "__main__":
    ...
    output_file_name = file_name.replace(".xlsx",".gpt-oss_enriched_output.xlsx") 
    ...
    for i, profilo in enumerate(profili):
        ...
        while attempt < MAX_RETRIES:
            ...
            temp_output = llm_enrich(profilo, llm, tokenizer)
        ...
        enriched_output["q17"] = q17_val[3:] if len(q17_val) > 3 and q17_val[1:3] in (": ", ". ") else q17_val
        enriched_output["q18"] = q18_val[3:] if len(q18_val) > 3 and q18_val[1:3] in (": ", ". ") else q18_val
        enriched_output["q19"] = q19_val[3:] if len(q19_val) > 3 and q19_val[1:3] in (": ", ". ") else q19_val
