...
def load_model() -> openai.OpenAI:
    ...
    if not AZURE_OPENAI_ENDPOINT or not AZURE_OPENAI_API_KEY:
        ...
        print("Azure OpenAI endpoint and API key found.")
        print(f"Endpoint: {AZURE_OPENAI_ENDPOINT}")
        print(f"API Key: {'*' * len(AZURE_OPENAI_API_KEY[:-4]) + AZURE_OPENAI_API_KEY[-4:]}")  
...
def llm_enrich(profilo: Dict[str, str], client: openai.OpenAI) -> Dict[str, object]:
...
D: Completamente d'accordo
    ...
    try:
        completion = client.chat.completions.create(
            model=MODEL_NAME,
            messages=[
                {"role": "system", "content": "Rispondi al questionario in formato JSON. Non aggiungere MAI testo aggiuntivo, spiegazioni o blocchi di codice markdown ('```json'). Devi ritornare solo l'oggetto JSON."},
    ...
    try:
        ...
        enriched_dict = json.loads(enriched)
        ...
        print(f"Raw output causing error: {enriched}")
        return {}
...
if __name__ == "__main__":
    ...
    output_file_name = file_name.replace(".xlsx",".gpt_enriched_output.xlsx") 
    ...
    try:
        ...
        print(f"❌ An unexpected error occurred during client loading: {e}")
        exit(1)
    ...
    for i, profilo in enumerate(profili):
        ...
        while attempt < MAX_RETRIES:
            ...
            temp_output = llm_enrich(profilo, client)
